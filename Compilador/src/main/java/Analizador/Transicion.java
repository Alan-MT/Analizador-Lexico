    ...
    public int nuevoEstado(char ch, int estadoActual) {
        ...
        if ((ch == ' ') | (ch == '\n')) {
            ...
            switch (estadoActual) {
                /**
                 *
                 * TABLA DE TRANSICIONES S0 -> letra -> S1 S0 -> '+' -> S6 S0 ->
                 * '(' -> S7 S1 -> letra -> S1 S0 -> dígito -> S2 S0 -> '-' ->
                 * S6 S0 -> ')' -> S7 S1 -> dígito -> S1 S0 -> '.' -> S5 S0 ->
                 * '*' -> S6 S0 -> '[' -> S7 S2 -> dígito -> S2 S0 -> ',' -> S5
                 * S0 -> '/' -> S6 S0 -> ']' -> S7 S2 -> '.' -> S3 S0 -> ';' ->
                 * S5 S0 -> '%' -> S6 S0 -> '{' -> S7 S3 -> dígito -> S4 S0 ->
                 * ':' -> S5 S0 -> '}' -> S7 S4 -> dígito -> S4
                 *
                 */
                case 0:
